    ...
    def __call__(self, inputs, state, emotion, imemory, scope=None):
        ...
        with vs.variable_scope(scope or 'GRUCell'):    # "GRUCell"
            with vs.variable_scope("Emotion_Imemory"):  # Reset gate and update gate.
                ...
                _r, _u, _c = array_ops.split(_linear(params,3 * self._num_units, False), 3, 1)
            with vs.variable_scope("Gates"):    # Reset gate and update gate.
                # We start with bias of 1.0 to not reset and not update.
                r, u = array_ops.split(_linear([inputs, state],2 * self._num_units, True, 1.0), 2, 1)
                r, u = sigmoid(r+_r), sigmoid(u+_u)
            with vs.variable_scope("Candidate"):
                c = self._activation(_c+_linear([inputs, r * state],self._num_units, True))
            new_h = u * state + (1 - u) * c 
...
class MEMMultiRNNCell(RNNCell):
    ...
    def __call__(self, inputs, state, emotion, imemory, scope=None):
        ...
        new_states = (tuple(new_states) if self._state_is_tuple
                                    else array_ops.concat(new_states,1))  #getshape
        new_imemory = imemory
